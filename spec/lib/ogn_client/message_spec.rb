...
describe OGNClient::Message do
  ...
  it "must parse valid raw message" do
    ...
    _(subject.callsign).must_equal "FLRDF0A52"
    _(subject.receiver).must_equal "LSTB"
    _(subject.time).must_equal Time.new(2012, 12, 12, 22, 01, 32, 0)
    _(subject.longitude).must_equal 7.128783
    _(subject.latitude).must_equal 46.978383
    _(subject.altitude).must_equal 434
    _(subject.heading).must_equal 90
    _(subject.ground_speed).must_equal 100
  end

  it "must parse valid raw message with explicit date" do
    raw = "FLRDF0A52>APRS,qAS,LSTB:/220132h4658.70N/00707.72Ez090/054/A=001424 !W37! id06DF0A52 +020fpm +0.0rot 55.2dB 0e -6.2kHz gps4x6 hearD7EA hearDA95"
    subject = OGNClient::Message.parse(raw, date: '2011-11-11')
    _(subject.time).must_equal Time.new(2011, 11, 11, 22, 01, 32, 0)
  end
  ...
  it "must parse valid raw message around midnight" do
    ...
    subject = OGNClient::Message.parse raw
    _(subject.time).must_equal Time.new(2012, 12, 11, 23, 59, 55, 0)
  end
  ...
  it 'must parse valid raw message without longitude nor latitude' do
    ...
    subject = OGNClient::Message.parse raw
    _(subject.longitude).must_be_nil
    _(subject.latitude).must_be_nil
  end
  ...
  it 'must parse valid raw message without heading nor ground speed' do
    ...
    subject = OGNClient::Message.parse raw
    _(subject.heading).must_be_nil
    _(subject.ground_speed).must_be_nil
  end
  ...
  it 'must parse valid raw message without altitude' do
    ...
    subject = OGNClient::Message.parse raw
    _(subject.altitude).must_be_nil
  end
  ...
  it "must parse valid raw message without longitude/latitude enhancement" do
    ...
    subject = OGNClient::Message.parse raw
    _(subject.longitude).must_equal 7.128667
    _(subject.latitude).must_equal 46.978333
  end

  it "must raise error for raw message class other than String" do
    raw = nil
    _{ OGNClient::Message.parse(raw) }.must_raise OGNClient::MessageError
  end
